...
function SortableSubtaskItem({ subtask, onToggle, renderSubtaskDueDate }: SortableSubtaskItemProps) {
  ...
  if (isDragging) {
    return (
      <div
        ref={setNodeRef}
        style={style}
        className="border-2 border-dashed border-blue-500 bg-blue-500/2 p-4 my-2.5 rounded-xl flex items-center justify-center gap-2 text-blue-500 font-extrabold text-xs select-none pointer-events-none w-full"
      >
        <span>Inserir subtarefa aqui</span>
      </div>
    );
  }
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-start gap-2 w-full text-left p-2 transition-colors rounded-lg group border-2 border-transparent hover:bg-gray-100 bg-transparent"
    >
      <button
        type="button"
      ...
      >
        <div
          className={`mt-0.5 shrink-0 ${subtask.completed ? "text-green-500" : "text-gray-300 group-hover:text-gray-400"}`}
        >
          {subtask.completed ? (
      ...
      <div
        {...attributes}
        {...listeners}
        className="mt-0.5 shrink-0 text-gray-400 hover:text-blue-600 cursor-grab active:cursor-grabbing p-0.5 rounded-md hover:bg-gray-200 transition-colors"
...
export function TaskCard({ task, index, totalTasks, onUpdate, onEditClick, onDetailClick, onMoveUp, onMoveDown, isOverlay }: TaskCardProps) {
  ...
  if (isDragging) {
    return (
      <div
        ref={setNodeRef}
        style={style}
        className="border-2 border-dashed border-blue-500 bg-blue-500/3 p-8 my-4 rounded-2xl flex flex-col items-center justify-center gap-2 min-h-[140px] pointer-events-none transition-all duration-200"
      >
        <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 animate-pulse">
          <CalendarClock size={18} className="stroke-[2.5]" />
        </div>
        <p className="text-[10px] font-black text-blue-600 tracking-wider uppercase select-none">Solte para posicionar tarefa</p>
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className={`glass-card mb-4 transition-all duration-300 ${isStatusMenuOpen ? 'relative z-50' : 'relative z-10'} ${
        isOverlay
          ? "ring-2 ring-primary-500 shadow-2xl pointer-events-none scale-[1.02]"
          : task.status === "Concluído"
          ? "bg-green-50/40 border-green-200/60 cursor-pointer hover:shadow-lg"
          : task.status === "Pendente"
          ? "bg-orange-50/30 border-orange-200/60 cursor-pointer hover:shadow-lg"
        ...
        <div className="flex justify-between items-start gap-4">
          ...
          <div className="flex items-center shrink-0" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="w-11 h-11 flex items-center justify-center text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-full transition-colors"
            ...
            <div
              {...attributes}
              {...listeners}
              className="w-11 h-11 flex items-center justify-center text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-full transition-colors cursor-grab active:cursor-grabbing"
              style={{ touchAction: "none" }}
